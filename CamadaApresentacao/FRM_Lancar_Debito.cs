...
{
    public partial class FRM_Lancar_Debito : Form
    {
        public bool eNovo;
        public int Num_Doc;

        //Codificação para evitar de abrir o Form 2X
        private static FRM_Lancar_Debito _Instancia;

        public static FRM_Lancar_Debito GetInstancia()
        {
            if (_Instancia == null)
            {
                _Instancia = new FRM_Lancar_Debito();
            }
            return _Instancia;
        }

        public FRM_Lancar_Debito()
        {
            InitializeComponent();
            this.TXB_Num_Doc.ReadOnly = true;
        }

        private void FRM_Lancar_Debito_FormClosed(object sender, FormClosedEventArgs e)
        {
            _Instancia = null;
        }
        ...
        private void BTN_Salvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.eNovo)
                {
                    resp = NLivro_Caixa.Inserir(this.DT_Data_Receb.Value, this.TXB_Historico.Text, this.TXB_Num_Doc.Text, Convert.ToDecimal("0,00"), Convert.ToDecimal(this.TXB_Valor.Text));
                }
                ...
                if (resp.Equals("Ok"))
                {
                    ...
                    resp = NLivro_Caixa.Atualizar_Num_Auto_Livro_Caixa_Debito(frm.Auxiliar_NA_Debito);
                }
                else
                {
