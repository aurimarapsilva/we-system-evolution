...
{
    public partial class FRM_Buscar_OS : Form
    {
        //Codificação para Evitar de abrir o mesmo form 2X.
        private static FRM_Buscar_OS _Instancia;

        public static FRM_Buscar_OS GetInstancia()
        {
            if (_Instancia == null)
            {
                _Instancia = new FRM_Buscar_OS();
            }
            return _Instancia;
        }

        public FRM_Buscar_OS()
        {
            InitializeComponent();
        ...
        private void FRM_Buscar_OS_Load(object sender, EventArgs e)
        {
            this.Mostrar();
        }
        ...
        private void FRM_Buscar_OS_FormClosed(object sender, FormClosedEventArgs e)
        {
            _Instancia = null;
        }
        ...
        private void dataLista_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string idos = this.dataLista.CurrentRow.Cells[1].Value.ToString();
            string tipo = this.dataLista.CurrentRow.Cells[4].Value.ToString();
            string num_os = this.dataLista.CurrentRow.Cells[3].Value.ToString();

            frm.Set_OS_Prenchida(idos, tipo, num_os);
            this.Close();
        }
    }
}
