...
{
    public partial class FRM_Equipamentos : Form
    {
        private bool eNovo = false;
        private bool eEditar = false;

        //Codificação para evitar de abrir o Form 2X
        private static FRM_Equipamentos _Instancia;

        public static FRM_Equipamentos GetInstancia()
        {
            if (_Instancia == null)
            {
                _Instancia = new FRM_Equipamentos();
            }
            return _Instancia;
        }

        public FRM_Equipamentos()
        {
            InitializeComponent();
            this.TT_Mensagem.SetToolTip(this.TXB_Nome, "Insira o nome do Equipamento");
        ...
        {
            this.DataLista.DataSource = NEquipamento.Mostrar();
            this.ocultarColunas();
        ...
        {
            this.DataLista.DataSource = NEquipamento.BuscarNome(this.TXB_Buscar.Text);
            this.ocultarColunas();
            LB_Total.Text = "Total de registros: " + Convert.ToString(DataLista.Rows.Count);
        }
        ...
        private void FRM_Equipamentos_Load(object sender, EventArgs e)
        {
            this.Mostrar();
            this.Habilitar(false);
            this.botoes();
        ...
        private void BTN_Salvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.eNovo)
                    {
                        resp = NEquipamento.Inserir(this.TXB_Nome.Text.Trim().ToUpper());
                    }
                    else
                    {
                        resp = NEquipamento.Editar(Convert.ToInt32(this.TXB_Id.Text), this.TXB_Nome.Text.Trim().ToUpper());
                    }
                ...
                if (Opcao == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in DataLista.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Resp = NEquipamento.Excluir(Convert.ToInt32(Codigo));
        ...
        private void FRM_Equipamentos_FormClosed(object sender, FormClosedEventArgs e)
        {
            _Instancia = null;

            FRM_OS frm = FRM_OS.GetInstancia();
            frm.ComboEquipamento();
        ...
        {
            this.TXB_Id.Text = Convert.ToString(this.DataLista.CurrentRow.Cells["idequipamento"].Value);
