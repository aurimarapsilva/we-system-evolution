...
{
    public partial class FRM_Detalhes_Cheque_Multiformas : Form
    {
        ...
        public FRM_Detalhes_Cheque_Multiformas()
        {
            InitializeComponent();
        }

        //Codificação para evitar de abrir o Form 2X
        private static FRM_Detalhes_Cheque_Multiformas _Instancia;

        public static FRM_Detalhes_Cheque_Multiformas GetInstancia()
        {
            if (_Instancia == null)
            {
                _Instancia = new FRM_Detalhes_Cheque_Multiformas();
            }
            return _Instancia;
        }
        ...
        private void FRM_Detalhes_Cheque_Multiformas_FormClosed(object sender, FormClosedEventArgs e)
        {
            _Instancia = null;

            FRM_Formas_Pgto_Venda frm = FRM_Formas_Pgto_Venda.GetInstancia();
            frm.Atualizar_Valores();
        }

        private void FRM_Detalhes_Cheque_Multiformas_Load(object sender, EventArgs e)
        {
            ...
            if (Opcao == DialogResult.OK)
            {
                ...
                else
                {
                    ...
                    if (this.num_parcela == this.quant_parcelas)
                    {
                        ...
                        frm.Adicionar_Dados_Grid_Formas_Pgto_Cheque(this.valor_total, this.valor_total);
                        this.Limpar();
                        this.Close();
                    }
                    else
