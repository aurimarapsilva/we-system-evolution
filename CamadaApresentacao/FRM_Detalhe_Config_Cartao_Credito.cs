...
{
    public partial class FRM_Detalhe_Config_Cartao_Credito : Form
    {
        private bool eAlterar = false;

        //Codificação para evitar de abrir o Form 2X
        private static FRM_Detalhe_Config_Cartao_Credito _Instancia;

        public static FRM_Detalhe_Config_Cartao_Credito GetInstancia()
        {
            if (_Instancia == null)
            {
                _Instancia = new FRM_Detalhe_Config_Cartao_Credito();
            }
            return _Instancia;
        }

        public FRM_Detalhe_Config_Cartao_Credito()
        {
            InitializeComponent();
        }
        ...
        {
            this.DGV_Config_Atual.DataSource = NDetalhe_Config_Cartao_Credito.Mostrar();
        ...
        private void FRM_Detalhe_Config_Cartao_Credito_Load(object sender, EventArgs e)
        {
            this.Mostrar();
        }
        ...
        private void BTN_Salvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.eAlterar)
                    {
                        resp = NDetalhe_Config_Cartao_Credito.Editar(Convert.ToInt32(this.TXB_Id.Text), this.CB_Compensar_Auto.Text);
                    }
        ...
        private void FRM_Detalhe_Config_Cartao_Credito_FormClosed(object sender, FormClosedEventArgs e)
        {
            _Instancia = null;
        }
    }
