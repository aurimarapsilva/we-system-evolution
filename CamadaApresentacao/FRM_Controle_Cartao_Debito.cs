...
{
    public partial class FRM_Controle_Cartao_Debito : Form
    {
        //Codificação para evitar de abrir o Form 2X
        private static FRM_Controle_Cartao_Debito _Instancia;

        public static FRM_Controle_Cartao_Debito GetInstancia()
        {
            if (_Instancia == null)
            {
                _Instancia = new FRM_Controle_Cartao_Debito();
            }
            return _Instancia;
        }

        public FRM_Controle_Cartao_Debito()
        {
            InitializeComponent();
        }
        ...
        {
            this.DGV_Dados.DataSource = NCartao_Debito.Mostrar();
            this.Formato_Grid();
        ...
        {
            this.DGV_Dados.DataSource = NCartao_Debito.Buscar_Datas(this.dtInicial.Value.ToString("dd/MM/yyyy"), this.dtFinal.Value.ToString("dd/MM/yyyy"));
            this.Formato_Grid();
            this.LB_Total_Registros.Text = Convert.ToString(this.DGV_Dados.Rows.Count);
        ...
        private void FRM_Controle_Cartao_Debito_Load(object sender, EventArgs e)
        {
            this.Mostrar();
        }
        ...
        private void FRM_Controle_Cartao_Debito_FormClosed(object sender, FormClosedEventArgs e)
        {
            _Instancia = null;
        }
        ...
        private void BTN_Ver_Origem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (chave)
                {
                    FRM_Ver_Venda_Origem_CDebito frm = FRM_Ver_Venda_Origem_CDebito.GetInstancia();
                    frm.idvenda = IdVenda;
                    frm.ShowDialog();
                }
                else
        ...
        private void BTN_Deletar_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult Opcao;
                Opcao = MessageBox.Show("Realmente deseja deletar este registro?", "WE System Evolution", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                if (Opcao == DialogResult.OK)
                {
                    string resp = "";

                    resp = NCartao_Debito.Excluir(Convert.ToInt32(Id));
                ...
                if (Opcao == DialogResult.OK)
                {
                    foreach (DataGridViewRow row in DGV_Dados.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            resp = NCartao_Debito.Excluir(Convert.ToInt32(IdRegistro));
                        }
                    }
                    if (chave)
                    {
                        ...
                        resp = NLivro_Caixa.Inserir(Data, "COMPENSAÇÃO DE CARTÃO DE DEBITO", Num_Doc, valor, Convert.ToDecimal("0,00"));
